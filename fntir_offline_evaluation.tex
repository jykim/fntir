...
\usepackage{color}
\newcommand{\paul}[1]{{\color{blue}\textit{Paul: #1}}}
...
without relying on actual users in a real-world environment. \paul{This suggests that lab studies are in scope.} Offline evaluation, notably test collection based evaluation, has been the dominant approach in IR evaluationand it is no exaggeration to say that shared evaluation efforts such as TREC have defined IR research over the years. The reason for this success lies in the ability to compare retrieval systems in a reusable manner.
