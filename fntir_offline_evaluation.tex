...
Judging target defines the basic unit and type of judgment. Judging unit would be determined by the goal of evaluation: i.e., in what granularity do we want to collect judgments? In web search, for example, judging unit can be a webpages, SERP elements or a whole SERP, as shown in Figure \ref{fig:judging_units}. 
...
Another consideration is the type of judgment, which can be either absolute or relative. Absolute judgment asks for judgiment about a single judging target, whereas relative judgment asks for pairwise preference between two judging targets. Figure \ref{fig:judgment_types} shows two types of judgments in evaluating web search results.
